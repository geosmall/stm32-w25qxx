...
static inline int w25qxx_stm32_spi_tx(void *ctx, const uint8_t *tx, size_t len)
{
    ...
    return HAL_SPI_Transmit(c->hspi, (uint8_t *)tx, (uint16_t)len, HAL_MAX_DELAY) != HAL_OK;
}
...
{
    W25qxxStm32Ctx *c = (W25qxxStm32Ctx *)ctx;
    return HAL_SPI_Receive(c->hspi, rx, (uint16_t)len, HAL_MAX_DELAY) != HAL_OK;
}
...
{
    W25qxxStm32Ctx *c = (W25qxxStm32Ctx *)ctx;
    HAL_GPIO_WritePin(c->cs_port, c->cs_pin, level ? GPIO_PIN_SET : GPIO_PIN_RESET);
}
